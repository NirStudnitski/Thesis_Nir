...
public class Vehicle : MonoBehaviour {
	...
	public float speed;
	private static float TWO_PI = 6.283185307f, HALF_PI = 0.78539816f, TO_DEG = 360f/TWO_PI;
	...
	public int lane;
	public int type;
	private bool turnInitiate = false, noCollision = true, turnDone = false;
	...
	void Update () {
		...
		if (!GameController.pause && noCollision) 
		{
			...
			else if (turnPlan == -1) 
			{
				...
				{
				case (0):
					if (!turnInitiate) if (transform.position.x > -33f)
						turnInitiate = true;
					...
					{
						float extra = transform.position.x + 33f;
						float arcLength = extra + speed * GameController.DELTA * turnCounter;
						theta = arcLength / GameController.rR;
						XYdir = RotateCW (new Vector2 (1, 0), theta);
				...
				case (2):
					if (!turnInitiate) if (transform.position.z < 33f)
						turnInitiate = true;
					...
					{
						float extra = 33f - transform.position.z;
						float arcLength = extra + speed * GameController.DELTA * turnCounter;
						theta = arcLength / GameController.rR;
						XYdir = RotateCW (new Vector2 (0, -1), theta);
				...
				case (4):
					...
					{
						float extra = 33f - transform.position.x;
						float arcLength = extra + speed * GameController.DELTA * turnCounter;
						theta = arcLength / GameController.rR;
						XYdir = RotateCW (new Vector2 (-1, 0), theta);
				...
				case (6):
					...
					{
						float extra = transform.position.z + 33f;
						float arcLength = extra + speed * GameController.DELTA * turnCounter;
						theta = arcLength / GameController.rR;
						XYdir = RotateCW (new Vector2 (0, 1), theta);
